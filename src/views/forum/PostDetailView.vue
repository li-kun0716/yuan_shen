  ...
  <div class="post-detail-app" ref="postDetailApp">
      ...
      <div class="content">
        ...
        <div class="info">
          <div>
            ...
            <span
              class="type"
              v-if="postDetail.user?.userAccount == '17883351224'"
              style="color: skyblue"
              >官方</span
            >
            <span
        ...
        <div class="publish-time">
          <span>文章发表：{{ postDetail.publishTime }}</span>
        </div>
        <div class="valueHtml" v-html="postDetail.postContent"></div>

        <ul class="toptic">
          <li
            v-for="(item, index) in JSON.parse(postDetail.postTopics || '[]')"
            :key="index"
          >
            {{ item }}
          </li>
    ...
    <div class="right">
      <div class="auth">
        <div class="avatar">
          <img
            v-lazy="postDetail.user.userAvatar"
            alt="avatar"
            v-if="postDetail.user"
          />
        </div>
        <div class="text">
      ...
      <div class="hot">
        ...
        <ul class="hot-list">
          ...
          >
            <div class="cover">
              <img
                v-lazy="JSON.parse(item.postCover || '[]')[0]"
                alt="coverImage"
              />
            </div>
            <div class="text">
...
const getPostInformation = () => {
  if (
    ...
    likedInfo.userAccount = JSON.parse(
      localStorage.getItem("token")
    ).userAccount;
...
watch(
  () => postLikedInfo.value.isLiked,
  (newValue) => {
    if (newValue) {
      Array.from(
        postDetailApp.value.querySelectorAll(".left .liked svg path")
      ).forEach((item) => {
...
const collectColor = computed(() =>
  collectInformation.value.isCollect === false ? "#cdcdcd" : "blue"
);
