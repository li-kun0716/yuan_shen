<template>
    <div class="some-person" ref="somePerson">
        <div class="mhy-layout">
            ...
            <div class="mhy-layout-upload" @click="gotoUploadSomeperson">
        ...
        <div class="some-person-inner">

            <div class="mhy-main">
                <div class="mhy-main-card-list">
                    <div class="card" v-for="item in somepersonList" :key="item.id" @click="getCreationDetail(item.id)">
                ...
                <div class="mhy-main-right">
                    ...
                    <div class="mhy-main-ranking">
                        <div class="rank-header">
                            <div class="rank-header-coterie">同人榜</div>
                            <span class="rank-header-more">更多</span>
                        </div>
                        <div>
                            <div class="rank-footer" v-for="(item, index) in hotsomepersonList.slice(0, 5)"
                                :key="item.id" @click="getCreationDetail(item.id)">
                                <div class="cover">
...
const somePerson = ref(null)
onMounted(() => {
    const navsObj = somePerson.value.querySelectorAll('.mhy-layout-tab')
...
const somepersonList = computed(() => _store.state.creation.somepersonList)
const hotsomepersonList = computed(() => [...somepersonList.value].sort((item1, item2) => item2.likedTotal - item1.likedTotal))
/**
 * 获取同人信息列表
 */
const getCreationListByType = (type) => {
    if (somepersonList.value.length == 0) {
        _store.dispatch('creation/getAllCreationByType', type)
    }
}
getCreationListByType('同人')
const _router = useRouter()
const gotoUploadSomeperson = () => {
    _router.push({
        name: 'creationUpload'
    })
}
...
const orderByHot = () => {
    somepersonList.value.sort((item1, item2) => item2.likedTotal - item1.likedTotal)

}
const orderByTime = () => {
    somepersonList.value.sort((item1, item2) => Date.parse(item2.publishTime.replace(/-/g, '/')) - Date.parse(item1.publishTime.replace(/-/g, '/')))

}
const orderByReply = () => {
    somepersonList.value.sort((item1, item2) => Date.parse(item2.latestReplyTime.replace(/-/g, '/')) - Date.parse(item1.latestReplyTime.replace(/-/g, '/')))
}
</script>
